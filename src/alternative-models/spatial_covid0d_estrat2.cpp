...
// A soma será a da linha i da matrix f(Dij)
double sum_beta_d(int k, int d, double y[][NA]) {
    double sum = 0.0;

    for (int j = 0; j < NEA; ++j)
    {
        sum +=  beta[day][k][j]*
            ((day - d >= 0 ? YOUT[k][Var::S][day - d] : 0)
             /(day - d - 1 >= 0 ? Y_sum[Var::N][day - d - 1] : Y_sum[Var::N][0]))
             *(day - d >= 0 ? YOUT[j][Var::I][day - d] : 0);
    }
...
void derivs_sir(double t, double y[][NA], double dydt[][NA])
{
    ...
    {
        printf("day: %d\n", day);
        printf("dif_beta: %lf\n",sum_beta(k, y) - sum_beta_d(k, dI[k], y));
        printf("sum_beta: %lf\n",sum_beta(k, y));
        //printf("sum_beta_d: %lf\n",sum_beta_d(k, dI[k], y));
        dydt[k][Var::S] = Lambda[k]*y[k][Var::N] - mu_eq[k] * y[k][Var::S] - sum_beta(k, y);
        
        dydt[k][Var::I] = sum_beta(k, y) 
            - mu_cov[day][k] * sum_beta_d(k, dC[k], y)
            - mu_eq[k] * y[k][Var::I]
            - gama[day][k] * sum_beta_d(k, dI[k], y);
        printf("didt: %lf\n", dydt[k][Var::I]);

        dydt[k][Var::R] = gama[day][k] * sum_beta_d(k, dI[k], y)
            - mu_eq[k] * y[k][Var::R];
...
void driver2D_simple(double y0[][NA], const char* filename)
{
    ...
    for (int i = 1; i < MAX_DAYS; i++)
    {
        day = i;
        derivs_sir(t, y, dydt);
        rk42D(y, dydt, t, h, yout, derivs_sir);
