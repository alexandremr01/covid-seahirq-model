...
pop_total = df.values[0, 1:]
natalidade = df.values[2, 1:]
pop_strata = np.divide(pop_total,np.sum(pop_total))
    ...
    def get_f(self):
        def f(t, y):
            ...
            for k in range(age_strata):
                ...
                df.append(natalidade[k]*N[k] - mu_eq[k] * S[k] - alpha_beta_row_sum - beta_row_sum - exp_beta_row_sum)                       # S
                df.append( beta_row_sum + alpha_beta_row_sum + exp_beta_row_sum  - (mu_eq[k] + a[k]) * E[k])                                 # E
                df.append( a[k] * rho[k] * E[k] - (theta[k] * mu_cov[k] + mu_eq[k] + gamma_H[k] + gamma_RI[k] + gamma_QI[k]) * I[k])          # I
                df.append( gamma_RI[k] * I[k] + gamma_RA[k] * A[k] + gamma_RQA[k] * Qa[k] + gamma_RQI[k] * Qi[k] + gamma_HR[k] * H[k] - mu_eq[k] * R[k]) # R
                
                df.append( natalidade[k] * N[k] - mu_eq[k] * N[k] - mu_cov[k] * H[k] - theta[k] * mu_cov[k] * I[k])     # N
                df.append( a[k] * (1.0 - rho[k]) * E[k] - (mu_eq[k] + gamma_RA[k] + gamma_QA[k]) * A[k]) # A
                df.append( mu_cov[k] * H[k]) # H
                df.append( theta[k] * mu_cov[k] *  I[k])
                df.append( gamma_H[k] * I[k] + gamma_HQI[k] * Qi[k] - (gamma_HR[k] + mu_eq[k] + mu_cov[k]) * H[k])
                df.append(0)
                df.append( gamma_QI[k] * I[k] - (gamma_HQI[k] + mu_eq[k] + gamma_RQI[k]) * Qi[k])
                df.append( gamma_QA[k] * A[k] - (gamma_RQA[k] + mu_eq[k]) * Qa[k])
...
scenarios = list(np.linspace(0.0, 2.0, 10))
base_case = np.zeros([192])
for k in range(age_strata):
    base_case[k] = pop_total[k]
    base_case[k+2] = 2*pop_strata[k]
    base_case[k+4] = pop_total[k]
base_case[12*12 + 2] = 1 # 1 infectado em I13
initial_conditions = [
    base_case
]
...
for i,param in enumerate(scenarios):
    ...
    for j,ic in enumerate(initial_conditions):
        # model.run(I0=15, R0=param, phases=EpidemicPhases(g0=1, itv0=0), verbose=False, attack=None, testing_parameters = TESTING_PARAMETERS_FROM_EPIDEMIOLOGY)
        ...
        # eq = df.values[:, 12:-18] 
        func = Func(param)
        sol = scipy.integrate.solve_ivp(func.get_f(), t_span=(0, 400), y0=ic,  t=time, first_step=0.1)
        trajectory[i,j] = sol.y
        print(trajectory[i,j].shape) 
equilibria = {}
x = []
y = []
for i, param in enumerate(scenarios):
    ...
    func = Func(param)
    flow = lambda y : func.get_f()(0, y)
    print(trajectory[i,j][-1,:].shape)
